...
Every assumption lives in ASSUMPTIONS below. The model is deliberately simple:
    ...
    net_income_2030       = base_revenue_2030 * base_margin_2030 + robotics_revenue_2030 * robotics_net_margin
    value_2030            = net_income_2030 * exit_pe
    implied_cagr          = (value_2030 / market_cap_today) ** (1 / years) - 1

Four unit scenarios are run. All money in USD millions unless noted. FX used for
conversion is fixed in FX (assumed, approximate Sep-2026 spot). Base-business
2030 income is an assumed margin on grown revenue; it is not linked to current
profits (base_ni_lc is kept for reference only). All bases, trailing or forward,
are compounded YEARS whether they are FY2025 actuals or FY2026/27 forecasts
(about +/-1pt of CAGR). Run:  python scripts/robotics_2030_model.py [--md OUT.md]
"""
...
YEARS = 4.25  # Sep 2026 -> end 2030; applied uniformly to trailing and forward bases

# 2030 humanoid units. Totals are anchored on Goldman's ~890k 2030 forecast (base);
# the Western and Tesla splits are our assumptions. The "broader embodied" multiplier
# was removed on 22 Sep 2026: after the Hesai fix no company used it.
SCENARIOS = {
    "crash": {"units": 150_000, "western_units": 30_000, "tesla_units": 15_000},
    "bear": {"units": 300_000, "western_units": 60_000, "tesla_units": 30_000},
    "base": {"units": 900_000, "western_units": 200_000, "tesla_units": 120_000},
...
class Co:
    ...
    base_ni_lc: float             # latest FY/TTM net income, local millions (reference only; not used; some entries are estimates)
    ...
    def units(self, s: dict) -> float:
        ...
        if self.pool == "western_ex_tesla":
            return (west - s["tesla_units"]) * self.use_west
        if self.pool == "tesla":
...
ASSUMPTIONS: list[Co] = [
    ...
    Co("Nvidia", "NVDA", "USD", 5_310_000, 411_000, 226_000, 0.15, 0.55, "western_ex_tesla", 2_000, 0.85, 0.55, 28,
       "Base = FY27 consensus $411B growing 15%; Jetson-class $2k per non-Tesla Western robot at 85% share (Tesla uses its own AI5; physical-AI cloud upsell excluded)"),
    ...
    Co("Qinchuan Machine Tool", "000837.SZ", "CNY", 9_140, 4_190, 50, 0.05, 0.03, "all", 160, 0.15, 0.12, 20,
       "Same grinder-capex pool ($160, corrected), 15% share; base business is low-margin machine tools; 50% of Chinese robots",
       use_cn=0.50),
    Co("LG Innotek", "011070.KS", "KRW", 12_160_000, 22_450_000, 485_000, 0.03, 0.025, "western_ex_tesla", 6 * 40, 0.50, 0.08, 10,
       "6 camera modules at $40 per non-Tesla Western robot, 50% share (Samsung Electro-Mechanics supplies Optimus)"),
    ...
    Co("Leaderdrive", "688017.SS", "CNY", 53_280, 571, 141, 0.15, 0.26, "chinese", 14 * 85, 0.50, 0.28, 30,
       "14 harmonics per Chinese-chain robot at $85, 50% share; only 50% of Chinese robots use harmonics (QDD designs do not). Deliberately conservative: reported Optimus harmonic supply (unverified) is not credited here",
       use_cn=0.50),
...
def breakeven_units(c: Co, target_cagr: float = 0.0) -> float | None:
    ...
    def cagr_at(u: float) -> float:
        k = u / base["units"]
        sc = {kk: v * k for kk, v in base.items()}
        r = c.project(sc)["cagr"]
...
def run(md_path: str | None = None) -> None:
    ...
    for i, c in enumerate(ASSUMPTIONS):
        ...
        f = lambda r: (("0%" if round(r['cagr']*100) == 0 else f"{r['cagr']*100:+.0f}%") if r['cagr'] == r['cagr'] else "n/m")
